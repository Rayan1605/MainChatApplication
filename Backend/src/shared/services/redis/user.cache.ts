...
export class UserCache extends BaseCache {
  ...
  public async updateSingleUserItemInCache(userId: string, prop: string, value: UserItem): Promise<IUserDocument | null> {
    try {
      console.log(`💾 Updating user cache: ${prop} for user ${userId}`);
      
      if (!this.client.isOpen) {
        console.log('🔌 Redis client not open, connecting...');
        await this.client.connect();
      }
      ...
      console.log('✅ Cache update successful, fetching updated user...');
      const response: IUserDocument = (await this.getUserFromCache(userId)) as IUserDocument;
      
      console.log('✅ User cache update completed successfully');
      return response;
    } catch (error) {
