...
export class Utils {
  ...
  static appImageUrl(version, id) {
    ...
    return `https://res.cloudinary.com/dfhkmkvtj/image/upload/v${version}/${id}`;
  }
  ...
  static getVideo(videoId, videoVersion) {
    if (typeof videoVersion === 'string' && typeof videoId === 'string') {
      videoVersion = videoVersion.replace(/['"]+/g, '');
      videoId = videoId.replace(/['"]+/g, '');
    }
    return videoVersion && videoId
      ? `https://res.cloudinary.com/dfhkmkvtj/video/upload/v${videoVersion}/${videoId}`
      : '';
  }
